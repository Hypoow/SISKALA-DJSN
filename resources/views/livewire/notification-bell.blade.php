<li class="nav-item dropdown app-navbar-item-notif nav-notif">
    ...
    <div class="dropdown-menu dropdown-menu-right shadow-lg border-0 notif-dropdown app-navbar-dropdown app-navbar-dropdown-notif"
        aria-labelledby="notificationDropdownTrigger" style="max-height: 480px; overflow-y: auto; border-radius: 12px;">
        <div
            class="dropdown-header bg-white border-bottom d-flex justify-content-between align-items-center py-3 px-4 sticky-top">
                    ...
                    <a href="javascript:void(0)" wire:click="markAsRead('{{ $uniqueId }}', '{{ $url }}')"
                        ...
                        <div class="mr-3">
                            <div class="avatar avatar-sm">
                                <span
                                    class="avatar-title rounded-circle shadow-sm {{ $iconBg }} {{ $isRead ? 'opacity-50' : '' }}">
                                    <i class="fe {{ $icon }}"></i>
                                </span>
                            </div>
                        </div>
                        ...
                        <div class="flex-fill">
                            <div class="d-flex justify-content-between align-items-center mb-1">
                                <span class="badge badge-pill {{ $badgeClass }} px-2 {{ $isRead ? 'opacity-50' : '' }}"
                                    style="font-size: 10px;">{{ $typeName }}</span>
                                <small class="text-muted {{ $isRead ? 'opacity-50' : '' }}"><i
                                        class="fe fe-clock mr-1"></i>{{ $date->isoFormat('D MMM') }}</small>
                            </div>

                            <h6 class="mb-1 {{ $isRead ? 'font-weight-normal text-muted' : 'font-weight-bold text-dark' }} text-truncate"
        ...
        @if($notifications->isNotEmpty())
            <div class="dropdown-footer text-center bg-light border-top py-2 d-flex justify-content-between px-4">
                <a href="javascript:void(0)" wire:click="clearHistory"
                    class="small font-weight-bold text-muted text-decoration-none">
                    <i class="fe fe-trash-2 mr-1"></i>Hapus Riwayat
                </a>
                <a href="javascript:void(0)" wire:click="markAllAsRead"
                    class="small font-weight-bold text-primary text-decoration-none">
