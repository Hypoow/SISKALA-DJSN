...
<div class="row justify-content-center">
    <div class="col-12">
        <h2 class="mb-2 page-title">Kegiatan Internal</h2>
        <p class="card-text">Daftar kegiatan internal yang akan datang dan telah terlaksana.</p>
                        ...
                        <div class="toolbar row mb-3">
                            ...
                            <div class="col ml-auto">
                                <div class="dropdown float-right">
                                    ...
                                    <a href="{{ route('activities.create', ['type' => 'internal']) }}" class="btn btn-primary float-right ml-3">Tambah Kegiatan +</a>
                                    @endif
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($activity->pic)
                                                @foreach($activity->pic as $pic)
                                                    @if($pic == 'Komjakum')
                                                        <span class="badge badge-danger">{{ $pic }}</span>
                                                    @elseif($pic == 'PME')
                                        ...
                                        <td>
                                            ...
                                            <div class="dropdown-menu dropdown-menu-right">
                                                {{-- Admin Only --}}
                                                @if(auth()->check() && auth()->user()->isAdmin())
                                                    <a class="dropdown-item" href="{{ route('internal-activities.edit', $activity->id) }}">Edit</a>
                                                    <form action="{{ route('internal-activities.destroy', $activity->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus kegiatan ini?');">
                                                        @csrf
                                                        @method('DELETE')
